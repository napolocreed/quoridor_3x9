...
\item \textbf{C4.} Exact partial bounds for the next open cell, \varSpec{4}{7}{7}: neither player forces a win within 26 plies; the central opening forces no first-player win within 27 (Appendix~\ref{sec:fourseven}).
\item \textbf{C5.} Measured move-ordering effects on certified branches (2--13$\times$ node reductions), a horizon-reversal caution against shallow pilot benchmarks, and four audited negative results (Appendix~\ref{sec:ordering}).
\end{itemize}
...
All solvers in this paper decide the bounded predicate $\mathrm{Win}(s,T,d)$: from state $s$, can player $T$ force a win within $d$ plies? The recursion is existential at $T$'s turns and universal at the opponent's, with terminal wins checked at entry and depth measured in plies (pseudocode in Appendix~\ref{app:pseudo}). Transposition entries store monotone facts (``$T$ wins within $d_1$'', ``$T$ does not win within $d_2$'')~\cite{zobrist1970new,knuth1975analysis}; keeping the remaining depth in the state avoids the graph-history-interaction pathology~\cite{kishimoto2004general}. Because a first-player win can end only on an odd ply, exact horizons follow from an upper bound at $d$ and an exhaustive refutation at $d-2$.

A note on draws and repetitions. The bounded predicate needs no repetition rule and no draw adjudication: $\mathrm{Win}(s,T,d)$ quantifies over \emph{all} continuations of at most $d$ plies, cyclic ones included---a line that revisits a position simply burns budget, which only hurts the attacker. Consequently the published claims are statements about $\mathrm{Win}$, not about any unbounded game value under a particular repetition or draw convention: ``first-player win in exactly 35 plies'' means $\mathrm{Win}(s_0,P_1,35)$ holds and $\mathrm{Win}(s_0,P_1,33)$ fails. Under any full-game semantics in which unbounded play without a win is not a win for Player~1 (draw or loss alike), these bounded facts transfer to ``Player~1 wins, and needs 35 plies against best defense''; the semantic boundary is stated here so that no such convention is smuggled in implicitly.
...
\subsection{Measurement conventions and environments}
\label{sec:measurement}

Throughout the paper, one \emph{node} is one entry into the bounded recursion $\mathrm{Win}(s,T,d)$---counted at function entry, before terminal, transposition, and pruning checks resolve the call---in both engines. The two engines' node counts are \emph{not} directly comparable: they differ in symmetry normalization, transposition policy, and move ordering, so cross-engine ratios measure architecture, not difficulty. For the verifiers, one \emph{expansion} (exhaustive C++ verifier) is one state whose local obligations are actually checked---states already verified at an equal or smaller remaining budget are memoized and not recounted---and one \emph{regenerated edge obligation} (streaming aggregate verifier) is one legal move regenerated from the independent rules engine and checked against the certificate's local obligations.

Principal environments, archived with the artifacts: the Engine~A conservative audits ran in a Debian container (g++ 14.2.0, \texttt{-O3 -DNDEBUG -std=c++20 -march=native}) on an AMD EPYC 9V74 slice (5 vCPU, 4~GiB cgroup memory limit); Engine~B production runs, the certificate pipeline, and the aggregate verification ran on a 12-core Windows~11 machine under MSYS2 g++ 16.1; the cross-toolchain reproduction machine ran Ubuntu 24.04 with g++ 13.3. The frozen specification documents are pinned by SHA-256 in the bundle manifest: \texttt{RULES.md} at \texttt{0f9a91d1a126fbd2$\ldots$} and \texttt{PROOF\_SEMANTICS.md} at \texttt{7f70c21e4f861edd$\ldots$} (full hashes in \texttt{MANIFEST.sha256} at the archived commit).
...
\section{Full Proof of the No-Stalemate Theorem}
\label{app:stalemateproof}

This appendix gives the complete proofs of the lemma and theorem of Section~\ref{sec:semantics}, following the archived theorem document (\texttt{emitter/STALEMATE\_THEOREM.md}) verbatim in substance. Wall-connectivity is the graph on cells whose edges are the orthogonally adjacent pairs not blocked by a wall; pawns never block edges of this graph (the rules define wall legality on unblocked cell edges; pawns are not walls, a pawn-transparent reading adopted identically by every engine in this paper).

\begin{proof}[Proof of the path invariant (Lemma 1)]
By induction over the move sequence. \emph{Base:} the initial position has no walls; the board is one component containing all rows. \emph{Wall placement:} legality explicitly requires that, after placement, each pawn has a path from its current cell to its own goal row---precisely the invariant. \emph{Pawn move:} walls are unchanged, so components are unchanged; it suffices to show the moving pawn stays in its component. A \emph{step} traverses an unblocked edge, so origin and destination are in one component. A \emph{straight jump} over the opponent at $n$ to $z$ requires the edges $p$--$n$ and $n$--$z$ unblocked, so $p$ and $z$ are connected through $n$. A \emph{diagonal} to a perpendicular neighbor $z$ of $n$ requires the approach edge $p$--$n$ and the connecting edge $n$--$z$ unblocked, so again $p$ and $z$ are connected through $n$. The non-moving pawn does not move.
\end{proof}
...
\begin{proof}[Proof of Theorem 1 (no stalemate, $H\geq 3$)]
Call a state \emph{legal} when the two pawns occupy distinct cells and each pawn's cell lies in the same wall-connectivity component as a cell of its goal row---the exact enumeration domain of the exhaustive audits, with no reachability requirement; by the lemma, every reachable state is legal. Suppose for contradiction that in some legal non-terminal state the player to move---call them $A$, pawn at $p$, goal row $g_A$---has no legal pawn move. Write $B$ for the opponent, pawn at $n$, goal row $g_B$; $\{g_A,g_B\}=\{0,H-1\}$.

\emph{Step 1: $p$ has at least one unblocked neighbor.} The state is non-terminal, so $p$ is not on row $g_A$. By legality, $p$'s component contains a cell of row $g_A$, hence a cell other than $p$, hence the component has at least two cells and $p$ has an unblocked edge.

\emph{Step 2: every unblocked neighbor of $p$ is occupied by $B$.} An empty unblocked neighbor is always a legal step. There is only one opponent, so $p$ has exactly one unblocked neighbor, namely $B$'s cell $n$.

\emph{Step 3: $n$'s only unblocked edge is $n$--$p$.} Since $A$ has no legal move: the straight jump fails, so the cell behind $n$ (in direction $p\to n$) is out of bounds or its edge from $n$ is blocked; by the rules, diagonals are then available, and both fail, so each perpendicular neighbor of $n$ is out of bounds or its edge from $n$ is blocked. The four potential edges at $n$ are: toward $p$ (unblocked, by Step~2), straight behind, and the two perpendiculars. Hence $n$'s unblocked edges are exactly $\{n\text{--}p\}$.

\emph{Step 4: the component of $p$ is exactly $\{p,n\}$.} By Steps 2 and 3.

\emph{Step 5: contradiction.} Applying legality to $A$: row $g_A$ intersects $\{p,n\}$; $p$ is not on $g_A$ (non-terminal), so $\mathrm{row}(n)=g_A$. Applying legality to $B$: row $g_B$ intersects $\{p,n\}$; $n$ is not on $g_B$ (non-terminal), so $\mathrm{row}(p)=g_B$. Thus $\{\mathrm{row}(p),\mathrm{row}(n)\}=\{0,H-1\}$. But $p$ and $n$ are orthogonally adjacent, so $|\mathrm{row}(p)-\mathrm{row}(n)|\leq 1$, forcing $H-1\leq 1$, i.e.\ $H\leq 2$: contradiction with $H\geq 3$.
\end{proof}

The theorem needs neither reachability nor stock consistency: it holds on the strict legal superset that the exhaustive audits enumerate (on \varSpec{4}{3}{3}: 63{,}184 legal non-terminal states versus 62{,}732 reachable---452 legal states are unreachable). It is also strictly stronger than ``no stalemate'': it guarantees a \emph{pawn} move even when the mover has walls in stock, so \texttt{stalemates\_seen}${}=0$ is a theorem for $H\geq 3$, not an empirical observation---though the counter stays in every audit as an implementation tripwire. Finally, the proof never uses the geometric wall-placement rules (no-cross, no-overlap), only connectivity, so it holds even for wall bitmasks that violate placement geometry but keep both pawns goal-connected. Sharpness at $H=2$ is witnessed by the reachable stalemate of Fig.~\ref{fig:stalemate}, which realizes exactly the two-cell prison of Step~4 with $\mathrm{row}(n)=g_A$ and $\mathrm{row}(p)=g_B$.

\section{Bounded Recursion and Certificate Obligations}
\label{app:pseudo}

\subsection{The bounded predicate}

Both engines decide $\mathrm{Win}(s,T,d)$ by the same recursion; they differ only in state representation, wall-legality decision, symmetry normalization, and transposition policy.

\begin{enumerate}[leftmargin=*]
\item If $s$ is terminal, return $[\text{winner}(s)=T]$.
\item If $d=0$, return \textbf{false}.
\item If the transposition table holds a monotone fact deciding the query---``$T$ wins within $d_1$'' with $d_1\leq d$, or ``$T$ does not win within $d_2$'' with $d_2\geq d$---return it.
\item Let $L(s)$ be the legal moves of $s$ ($L(s)\neq\emptyset$ for pawn moves by Theorem~1, so the empty-conjunction branch is dead code for $H\geq 3$).
\item If the player to move is $T$: result $\leftarrow \bigvee_{m\in L(s)}\mathrm{Win}(\mathrm{apply}(s,m),T,d-1)$; otherwise result $\leftarrow \bigwedge_{m\in L(s)}\mathrm{Win}(\mathrm{apply}(s,m),T,d-1)$.
\item Store the monotone fact implied by the result at depth $d$; return it.
\end{enumerate}

One \emph{node} is one entry into this recursion (Section~\ref{sec:measurement}). Symmetry normalization maps $s$ to a canonical representative before steps 3 and 6; the conservative audits disable the admissible distance bound and the pawn-only endgame table, so the recursion above is exactly what they execute.

\subsection{\textsf{qcert-1} verification obligations}

A certificate for ``player $T$ wins from root $r$ within $d$ plies'' is a set of records $(s,\mathit{rank}_s,\mathit{move}_s)$, with $\mathit{move}_s$ present exactly when $T$ moves at $s$. The verifier accepts iff all of the following hold; every obligation is local to one record and its regenerated successors:

\begin{enumerate}[leftmargin=*]
\item \emph{Root:} $r$ is present with $\mathit{rank}_r\leq d$.
\item \emph{$T$-node:} $\mathit{move}_s$ is legal in $s$ (regenerated from the rules engine), and $\mathrm{apply}(s,\mathit{move}_s)$ is either an immediate terminal win for $T$ or present with strictly smaller rank.
\item \emph{Opponent node:} \emph{every} legal reply $m\in L(s)$ leads to a state that is an immediate terminal win for $T$ or present with strictly smaller rank.
\item \emph{Terminal:} every terminal state in the certificate is a win for $T$.
\item \emph{Well-foundedness:} ranks are nonnegative integers, so strict decrease along every certified line is a syntactic property---no cycle can certify---and the ply bound is arithmetic on ranks.
\end{enumerate}

Soundness is immediate by induction on $\mathit{rank}_s$: at every certified state, $T$ either wins immediately or forces play into certified states of strictly smaller rank, hence wins within $\mathit{rank}_s$ plies. The verifier never searches: it enumerates each record's obligations once, in any order.
...
\section{Ordering Portfolios, Race Endgames, and Negative Results}
\label{sec:ordering}
