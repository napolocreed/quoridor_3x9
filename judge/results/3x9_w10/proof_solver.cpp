...
public:
    ...
    ProofSolver(const Engine& eng,unsigned tt_bits):e(eng),tt(tt_bits) {
    ...
    bool prove(const State& s,Player target,int depth,
        ...
        // Even with a perfectly cooperative opponent, one pawn move traverses at most two graph edges.
        // If the target cannot physically reach its goal in its remaining turns, this branch is dead.
        int ti=int(target), oi=1-ti;
        int graph_d=e.dist[ti][size_t(s.cfg)*e.N+s.p[ti]];
        // Before the first jump, two non-adjacent pawns can close Manhattan distance by at most
        // one cell each per round. Once contact is possible we optimistically allow a jump on
        // every target move. This remains a relaxation of the real game, hence an admissible bound.
        ...
        if(separated) min_target_moves=graph_d; // walls never reconnect components, hence no future jump is possible
        else {
            if(s.turn!=target && contact_lb>1)--contact_lb; // cooperative opponent gets one move before target
            int pre_jump=std::max(1,contact_lb)/2; // normal target moves before the first possible jump
            int jump_plan=std::max(pre_jump+1,(graph_d+pre_jump+1)/2);
            min_target_moves=std::min(graph_d,jump_plan); // winning before contact may be faster
        }
...
int main(int argc,char**argv){
    ...
    int W=3,H=9,walls=8,max_depth=80,start_depth=1,tt_bits=24,order_mode=1,target_filter=-1,root_index=-1,root_index2=-1,jobs=3,child_depth=-1;double seconds=60;bool precompute_only=false,list_root=false,list_second=false,parallel_second=false;
    for(int i=1;i<argc;i++){
        ...
        else if(a=="--seconds")val(seconds);else if(a=="--tt-bits")val(tt_bits);else if(a=="--order")val(order_mode);else if(a=="--precompute-only")precompute_only=true;
        else if(a=="--help"){std::cout<<"frontier_solver --width 3 --height 9 --walls 8 --seconds 600 --max-depth 120 --tt-bits 25\n";return 0;}
    }
    try{
        ...
        if(parallel_second) {
            ...
            auto launch=[&](int ti){
                ...
                if(pid==0){close(fds[0]);ProofSolver ps(e,tt_bits);auto r=ps.solve_from(tasks[ti].st,seconds,child_depth,std::max(1,child_depth-2),int(target));
        ...
        ProofSolver ps(e,tt_bits);auto r=ps.solve_from(root,seconds,max_depth,start_depth,target_filter);
