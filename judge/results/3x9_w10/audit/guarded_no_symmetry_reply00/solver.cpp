...
public:
    ...
    void record(uint64_t key, int depth, bool value, uint16_t best_move) {
        ...
        if(e->win_depth!=0xffff && e->fail_depth>=e->win_depth)
            throw std::runtime_error("inconsistent transposition bounds");
...
public:
    ...
    Engine(int width, int height, int walls, int ordering = 1, int symmetry = 2)
...
public:
    ...
    bool prove(const State& s,Player target,int depth,
        ...
        if(n==0){record_tt(key,depth,false,0xffff);return false;}
        if(hint!=0xffff) {
...
int main(int argc,char**argv){
    ...
    int W=3,H=9,walls=8,max_depth=80,start_depth=1,tt_bits=24,order_mode=1,target_filter=-1,root_index=-1,root_index2=-1,jobs=3,child_depth=-1;double seconds=60;bool precompute_only=false,list_root=false,list_second=false,parallel_second=false,use_bounds=true,use_pawn_table=true,use_tt=true,print_pv=false;int symmetry_mode=2;
    ...
    try{
        Engine e(W,H,walls,order_mode,symmetry_mode);e.precompute(false);
