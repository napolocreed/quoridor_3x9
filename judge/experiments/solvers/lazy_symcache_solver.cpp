...
struct State {
    ...
    uint32_t cfg_id = 0;
    uint8_t cfg_xform = 0; // original orientation -> canonical wall orientation
    uint8_t p[2]{};
    uint8_t walls[2]{};
    Player turn = P1;
};
...
private:
    FlatMaskIndex raw_index_;
    FlatMaskIndex canonical_index_;
    ...
    size_t raw_index_size_=0;
    std::vector<uint64_t> node_visits_;
    uint64_t cache_hits_=0,cache_misses_=0,canonical_hits_=0,canonical_misses_=0;

public:
    struct ConfigRef { uint32_t id; uint8_t xform; };

    LazyEngine(int width,int height,int walls,int ordering=1,int symmetry=2,size_t reserve_configs=65536,bool profile_visits=false)
        :W(width),H(height),C(width-1),R(height-1),S(C*R),N(width*height),walls_each(walls),
         max_total_walls(std::min(2*walls,2*C*R)), raw_index_(reserve_configs), canonical_index_(std::max<size_t>(1024,reserve_configs/2)) {
        order_mode=ordering;symmetry_mode=symmetry;profile_visits_=profile_visits;
        ...
        init_geometry();
        cache_data_.reserve(reserve_configs);if(profile_visits_)node_visits_.reserve(reserve_configs);
        ConfigRef empty=ensure_config(0);
        if(empty.id!=0||empty.xform!=0) throw std::runtime_error("empty config must receive canonical id zero");
    }
    ...
    size_t raw_cache_size()const{return raw_index_size_;}
    uint64_t cache_hits()const{return cache_hits_;}
    uint64_t cache_misses()const{return cache_misses_;}
    uint64_t canonical_hits()const{return canonical_hits_;}
    uint64_t canonical_misses()const{return canonical_misses_;}
    size_t cache_payload_bytes()const{return cache_data_.capacity()*sizeof(ConfigData);}
    size_t cache_estimated_bytes()const{return cache_payload_bytes()+raw_index_.bytes()+canonical_index_.bytes()+node_visits_.capacity()*sizeof(uint64_t);}
    ...
    ConfigRef ensure_config(uint64_t mask) {
        uint32_t packed=raw_index_.find(mask);
        if(packed!=UINT32_MAX){++cache_hits_;return ConfigRef{packed>>2,uint8_t(packed&3)};}
        ++cache_misses_;
        std::array<uint64_t,4> variants{mask,transform_lut(mask,mirror_lut_),transform_lut(mask,rotate_lut_),transform_lut(mask,mirror_rotate_lut_)};
        uint8_t xf=0;
        for(uint8_t i=1;i<4;i++) if(variants[i]<variants[xf]) xf=i;
        uint64_t canonical=variants[xf];
        uint32_t id=canonical_index_.find(canonical);
        if(id==UINT32_MAX){
            ++canonical_misses_;
            id=uint32_t(cache_data_.size());
            cache_data_.push_back(build_config(canonical));
            if(profile_visits_) node_visits_.push_back(0);
            canonical_index_.insert(canonical,id);
        }else ++canonical_hits_;
        packed=(id<<2)|xf;
        raw_index_.insert(mask,packed);
        ++raw_index_size_;
        return ConfigRef{id,xf};
    }
    const ConfigData& config_by_id(uint32_t id) const { return cache_data_[id]; }
    inline uint8_t transform_square(uint8_t p,uint8_t xf)const{
        if(xf==0)return p;
        if(xf==1)return mirror_sq[p];
        if(xf==2)return rotate_sq[p];
        return mirror_sq[rotate_sq[p]];
    }
    inline int transform_direction(int d,uint8_t xf)const{
        static constexpr uint8_t map[4][4]={{0,1,2,3},{0,1,3,2},{1,0,3,2},{1,0,2,3}};
        return map[xf][d];
    }
    inline int distance_for(const State&s,int original_player,uint8_t original_square,const ConfigData&cd)const{
        int canonical_player=original_player^(s.cfg_xform>=2);
        uint8_t canonical_square=transform_square(original_square,s.cfg_xform);
        return canonical_player==0?cd.dist0[canonical_square]:cd.dist1[canonical_square];
    }
    inline uint64_t legal_add_for(const State&s,const ConfigData&cd)const{
        if(s.cfg_xform==0)return cd.legal_add;
        if(s.cfg_xform==1)return transform_lut(cd.legal_add,mirror_lut_);
        if(s.cfg_xform==2)return transform_lut(cd.legal_add,rotate_lut_);
        return transform_lut(cd.legal_add,mirror_rotate_lut_);
    }
    inline bool clear_edge_for(const State&s,const ConfigData&cd,uint8_t p,int d)const{
        uint8_t cp=transform_square(p,s.cfg_xform);
        int dd=transform_direction(d,s.cfg_xform);
        return ((cd.dir[dd]>>cp)&1u)!=0;
    }
    ...
    int static_eval_data(const State&s,Player target,const ConfigData&cd) const {
        ...
        int dt=distance_for(s,t,s.p[t],cd);
        int do_=distance_for(s,o,s.p[o],cd);
    ...
    int generate(const State&s,Player target,Child*out,bool ordered=true){
        ...
        auto clear_at=[&](uint8_t p){uint8_t z=0;for(int d=0;d<4;d++)if(clear_edge_for(s,current,p,d))z|=uint8_t(1u<<d);return z;};
        ...
        if(s.walls[me]>0){
            uint64_t adds=legal_add_for(s,current);
            int before_op=distance_for(s,op,s.p[op],current);
            int before_me=distance_for(s,me,s.p[me],current);
            while(adds){int w=std::countr_zero(adds);adds&=adds-1;uint64_t nm=s.walls_mask|(1ULL<<w);ConfigRef nr=ensure_config(nm);const auto&nx=config_by_id(nr.id);
                State t=s;t.walls_mask=nm;t.cfg_id=nr.id;t.cfg_xform=nr.xform;
                if(distance_for(t,0,t.p[0],nx)==255||distance_for(t,1,t.p[1],nx)==255)continue;
                --t.walls[me];t.turn=other(s.turn);
                int sc=static_eval_data(t,target,nx);
                int after_op=distance_for(t,op,t.p[op],nx);
                int after_me=distance_for(t,me,t.p[me],nx);
    ...
    bool structurally_saturated(uint64_t mask){ConfigRef r=ensure_config(mask);State s;s.walls_mask=mask;s.cfg_id=r.id;s.cfg_xform=r.xform;return legal_add_for(s,config_by_id(r.id))==0;}
    ...
    uint64_t canonical_key(const State&s,Player target,int depth,bool cap_stocks)const{
        ...
        uint64_t a=pack(s.walls_mask,s.p[0],s.p[1],w0,w1,s.turn,target);if(symmetry_mode==0)return a;
        uint64_t mm=transform_lut(s.walls_mask,mirror_lut_);uint64_t b=pack(mm,mirror_sq[s.p[0]],mirror_sq[s.p[1]],w0,w1,s.turn,target);if(symmetry_mode==1)return std::min(a,b);
        uint64_t rm=transform_lut(s.walls_mask,rotate_lut_);uint64_t c=pack(rm,rotate_sq[s.p[1]],rotate_sq[s.p[0]],w1,w0,other(s.turn),other(target));
        uint64_t rmm=transform_lut(s.walls_mask,mirror_rotate_lut_);uint64_t d=pack(rmm,mirror_sq[rotate_sq[s.p[1]]],mirror_sq[rotate_sq[s.p[0]]],w1,w0,other(s.turn),other(target));
...
private:
    void init_geometry(){
        ...
        std::array<std::array<uint32_t,4>,40> wall_block{};
        for(int r=0;r<R;r++)for(int c=0;c<C;c++){
            int h=slot(r,c),v=S+slot(r,c);
            wall_block[h][1]|=(1u<<sq(r,c))|(1u<<sq(r,c+1));
            wall_block[h][0]|=(1u<<sq(r+1,c))|(1u<<sq(r+1,c+1));
            wall_block[v][3]|=(1u<<sq(r,c))|(1u<<sq(r+1,c));
            wall_block[v][2]|=(1u<<sq(r,c+1))|(1u<<sq(r+1,c+1));
        }
        for(int ch=0;ch<4;ch++)for(int v=0;v<1024;v++)for(int bit=0;bit<10;bit++){
            int w=ch*10+bit;
            if(w>=2*S) break;
            if(v&(1<<bit)){
                for(int d=0;d<4;d++) block_lut_[ch][v][d]|=wall_block[w][d];
...
class ProofSolver {
    ...
    LazyEngine&e;TransTable tt;bool use_tt=true;bool cap_stocks=false;int local_threshold=0,local_max=0,local_count=0;size_t local_entries=0;std::vector<LocalSlot> local_slots;uint64_t local_hits=0,local_probes=0;Clock::time_point deadline;uint64_t nodes=0,tt_hits=0,cutoffs=0;
...
public:
    struct Result{bool solved=false;Player winner=P1;int depth=-1;uint64_t nodes=0,tt_hits=0,cutoffs=0,local_hits=0,local_probes=0;int local_tables=0;double seconds=0;bool timeout=false;};
    ProofSolver(LazyEngine&eng,unsigned tt_bits,bool transpositions=true,bool horizon_stock_cap=false,int tt_ways=2,int local_tt_threshold=0,int local_tt_max=0):e(eng),tt(transpositions?tt_bits:10,tt_ways),use_tt(transpositions),cap_stocks(horizon_stock_cap),local_threshold(local_tt_threshold),local_max(local_tt_max){if(local_tt_threshold>0)throw std::runtime_error("local TT is disabled in symmetry-cache prototype");local_entries=size_t(e.walls_each+1)*(e.walls_each+1)*2*e.N*e.N;std::cerr<<"TT="<<(use_tt?LazyEngine::human_bytes(tt.bytes()):std::string("disabled"))<<" ways="<<tt_ways<<" lazy_cache stock_cap="<<cap_stocks<<" local_threshold="<<local_threshold<<" local_max="<<local_max<<" local_table="<<LazyEngine::human_bytes(local_entries*4)<<"\n";}
    ...
    bool prove(const State&s,Player target,int depth,std::vector<std::array<LazyEngine::Child,192>>&lists,int ply){
        ...
        auto&ch=lists[ply];int n=e.generate(s,target,ch.data(),true);if(n==0){if(use_tt)tt.record(key,depth,false,0xffff);if(lt)lt->record(li,depth,false,0xffff);return false;}if(hint!=0xffff)for(int i=0;i<n;i++)if(ch[i].move==hint){if(i)std::swap(ch[0],ch[i]);break;}
    ...
    done:out.nodes=nodes;out.tt_hits=tt_hits;out.cutoffs=cutoffs;out.local_hits=local_hits;out.local_probes=local_probes;out.local_tables=local_count;out.seconds=std::chrono::duration<double>(Clock::now()-t0).count();return out;}
};

} // namespace qsym

int main(int argc,char**argv){using namespace qsym;int W=3,H=9,walls=8,max_depth=80,start_depth=1,tt_bits=24,tt_ways=2,local_tt_threshold=0,local_tt_max=0,order=1,target_filter=-1,root_index=-1,root_index2=-1,root_index3=-1;double seconds=60;bool list_root=false,list_second=false,list_third=false,use_tt=true,horizon_stock_cap=false,profile_visits=false;int symmetry=2;size_t cache_reserve=65536;
    ...
    try{LazyEngine e(W,H,walls,order,symmetry,cache_reserve,profile_visits);State root=e.initial();auto label=[&](uint16_t mv){std::ostringstream o;if(mv<0x100){o<<"P("<<e.row(uint8_t(mv))<<","<<e.col(uint8_t(mv))<<")";}else{int w=mv-0x100,ori=w/e.S,z=w%e.S;o<<(ori?"V":"H")<<"("<<z/e.C<<","<<z%e.C<<")";}return o.str();};
        ...
        ProofSolver ps(e,tt_bits,use_tt,horizon_stock_cap,tt_ways,local_tt_threshold,local_tt_max);auto r=ps.solve_from(root,seconds,max_depth,start_depth,target_filter);std::cout<<"width="<<W<<" height="<<H<<" walls="<<walls<<" solved="<<r.solved;if(r.solved)std::cout<<" winner="<<(r.winner==P1?1:2)<<" depth="<<r.depth;std::cout<<" nodes="<<r.nodes<<" tt_hits="<<r.tt_hits<<" cutoffs="<<r.cutoffs<<" local_hits="<<r.local_hits<<" local_probes="<<r.local_probes<<" local_tables="<<r.local_tables<<" seconds="<<r.seconds<<" timeout="<<r.timeout<<" cache_configs="<<e.cache_size()<<" raw_configs="<<e.raw_cache_size()<<" cache_hits="<<e.cache_hits()<<" cache_misses="<<e.cache_misses()<<" canonical_hits="<<e.canonical_hits()<<" canonical_misses="<<e.canonical_misses()<<" cache_payload="<<e.cache_payload_bytes()<<" cache_estimated="<<e.cache_estimated_bytes();if(profile_visits)std::cout<<" "<<e.visit_profile();std::cout<<"\n";
