...
public:
    ...
    size_t cache_estimated_bytes()const{return cache_payload_bytes()+cache_index_.bytes()+node_visits_.capacity()*sizeof(uint64_t);}
    ...
    int generate(const State&s,Player target,Child*out,bool ordered=true){
        ...
        if(s.walls[me]>0){
            ...
            while(adds){int w=std::countr_zero(adds);adds&=adds-1;uint64_t nm=s.walls_mask|(1ULL<<w);uint32_t nid=ensure_child_config(nm,s.cfg_id,w);const auto&nx=config_by_id(nid);
                if(nx.dist0[s.p[0]]==255||nx.dist1[s.p[1]]==255)continue;
                State t=s;t.walls_mask=nm;t.cfg_id=nid;--t.walls[me];t.turn=other(s.turn);
...
public:
    ...
    ProofSolver(LazyEngine&eng,unsigned tt_bits,bool transpositions=true,bool horizon_stock_cap=false,int tt_ways=2,int local_tt_threshold=0,int local_tt_max=0):e(eng),tt(transpositions?tt_bits:10,tt_ways),use_tt(transpositions),cap_stocks(horizon_stock_cap),local_threshold(local_tt_threshold),local_max(local_tt_max){local_entries=size_t(e.walls_each+1)*(e.walls_each+1)*2*e.N*e.N;std::cerr<<"TT="<<(use_tt?LazyEngine::human_bytes(tt.bytes()):std::string("disabled"))<<" ways="<<tt_ways<<" lazy_cache stock_cap="<<cap_stocks<<" local_threshold="<<local_threshold<<" local_max="<<local_max<<" local_table="<<LazyEngine::human_bytes(local_entries*4)<<"\n";}
