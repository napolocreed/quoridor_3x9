...
public:
    LazyEngine(int width,int height,int walls,int ordering=1,int symmetry=2,size_t reserve_configs=65536,bool profile_visits=false,int transition_threshold=0)
        ...
        order_mode=ordering;symmetry_mode=symmetry;profile_visits_=profile_visits;transition_threshold_=transition_threshold;
...
class ProofSolver {
    ...
    LazyEngine&e;TransTable tt;bool use_tt=true;bool cap_stocks=false;bool stock_dominance=false;int dominance_radius=99;int local_threshold=0,local_max=0,local_count=0;size_t local_entries=0;std::vector<LocalSlot> local_slots;uint64_t local_hits=0,local_probes=0,dominance_probes=0,dominance_hits=0;Clock::time_point deadline;uint64_t nodes=0,tt_hits=0,cutoffs=0;
...
public:
    struct Result{bool solved=false;Player winner=P1;int depth=-1;uint64_t nodes=0,tt_hits=0,cutoffs=0,local_hits=0,local_probes=0,dominance_probes=0,dominance_hits=0;int local_tables=0;double seconds=0;bool timeout=false;};
    ProofSolver(LazyEngine&eng,unsigned tt_bits,bool transpositions=true,bool horizon_stock_cap=false,int tt_ways=2,int local_tt_threshold=0,int local_tt_max=0,bool enable_stock_dominance=false,int stock_dominance_radius=99):e(eng),tt(transpositions?tt_bits:10,tt_ways),use_tt(transpositions),cap_stocks(horizon_stock_cap),stock_dominance(enable_stock_dominance),dominance_radius(stock_dominance_radius),local_threshold(local_tt_threshold),local_max(local_tt_max){local_entries=size_t(e.walls_each+1)*(e.walls_each+1)*2*e.N*e.N;std::cerr<<"TT="<<(use_tt?LazyEngine::human_bytes(tt.bytes()):std::string("disabled"))<<" ways="<<tt_ways<<" lazy_cache stock_cap="<<cap_stocks<<" local_threshold="<<local_threshold<<" local_max="<<local_max<<" local_table="<<LazyEngine::human_bytes(local_entries*4)<<"\n";}
    ...
    bool prove(const State&s,Player target,int depth,std::vector<std::array<LazyEngine::Child,192>>&lists,int ply){
        ...
        if(use_tt&&stock_dominance){
            int t=int(target),o=1-t,total=int(s.walls[0])+int(s.walls[1]);
            int own=int(s.walls[t]);
            int min_own=std::max(0,total-e.walls_each),max_own=std::min(e.walls_each,total);
            State alt=s;bool dv;uint16_t dh;
            for(int a=own-1;a>=min_own&&own-a<=dominance_radius;--a){alt.walls[t]=uint8_t(a);alt.walls[o]=uint8_t(total-a);++dominance_probes;uint64_t dk=e.canonical_key(alt,target,depth,cap_stocks);if(tt.probe(dk,depth,dv,dh)&&dv){++dominance_hits;return true;}}
            alt=s;
            for(int a=own+1;a<=max_own&&a-own<=dominance_radius;++a){alt.walls[t]=uint8_t(a);alt.walls[o]=uint8_t(total-a);++dominance_probes;uint64_t dk=e.canonical_key(alt,target,depth,cap_stocks);if(tt.probe(dk,depth,dv,dh)&&!dv){++dominance_hits;return false;}}
        }
    ...
    done:out.nodes=nodes;out.tt_hits=tt_hits;out.cutoffs=cutoffs;out.local_hits=local_hits;out.local_probes=local_probes;out.local_tables=local_count;out.dominance_probes=dominance_probes;out.dominance_hits=dominance_hits;out.seconds=std::chrono::duration<double>(Clock::now()-t0).count();return out;}
};

} // namespace qdom

int main(int argc,char**argv){using namespace qdom;int W=3,H=9,walls=8,max_depth=80,start_depth=1,tt_bits=24,tt_ways=2,transition_cache_threshold=0,dominance_radius=99,local_tt_threshold=0,local_tt_max=0,order=1,target_filter=-1,root_index=-1,root_index2=-1,root_index3=-1;double seconds=60;bool list_root=false,list_second=false,list_third=false,use_tt=true,horizon_stock_cap=false,profile_visits=false,stock_dominance=false;int symmetry=2;size_t cache_reserve=65536;
    for(int i=1;i<argc;i++){std::string a=argv[i];auto val=[&](auto&x){if(i+1>=argc)throw std::runtime_error("missing arg");std::stringstream ss(argv[++i]);ss>>x;};
        if(a=="--width")val(W);else if(a=="--height")val(H);else if(a=="--walls")val(walls);else if(a=="--max-depth")val(max_depth);else if(a=="--start-depth")val(start_depth);else if(a=="--target")val(target_filter);else if(a=="--tt-bits")val(tt_bits);else if(a=="--tt-ways")val(tt_ways);else if(a=="--transition-cache-threshold")val(transition_cache_threshold);else if(a=="--stock-dominance-radius")val(dominance_radius);else if(a=="--local-tt-threshold")val(local_tt_threshold);else if(a=="--local-tt-max")val(local_tt_max);else if(a=="--order")val(order);else if(a=="--seconds")val(seconds);else if(a=="--root-index")val(root_index);else if(a=="--root-index2")val(root_index2);else if(a=="--root-index3")val(root_index3);else if(a=="--list-root")list_root=true;else if(a=="--list-second")list_second=true;else if(a=="--list-third")list_third=true;else if(a=="--no-tt")use_tt=false;else if(a=="--no-symmetry")symmetry=0;else if(a=="--mirror-only")symmetry=1;else if(a=="--cache-reserve")val(cache_reserve);else if(a=="--horizon-stock-cap")horizon_stock_cap=true;else if(a=="--stock-dominance")stock_dominance=true;else if(a=="--profile-config-visits")profile_visits=true;else if(a=="--no-bounds"||a=="--no-pawn-table"){}else if(a=="--help"){std::cout<<"lazy_frontier_solver --width 4 --height 7 --walls 7 --root-index 0 --root-index2 0 --root-index3 8 --target 1 --start-depth 24 --max-depth 24 --tt-bits 27\n";return 0;}else throw std::runtime_error("unknown argument: "+a);}
    if(target_filter==1)target_filter=0; else if(target_filter==2)target_filter=1; else target_filter=-1;
    try{LazyEngine e(W,H,walls,order,symmetry,cache_reserve,profile_visits,transition_cache_threshold);State root=e.initial();auto label=[&](uint16_t mv){std::ostringstream o;if(mv<0x100){o<<"P("<<e.row(uint8_t(mv))<<","<<e.col(uint8_t(mv))<<")";}else{int w=mv-0x100,ori=w/e.S,z=w%e.S;o<<(ori?"V":"H")<<"("<<z/e.C<<","<<z%e.C<<")";}return o.str();};
