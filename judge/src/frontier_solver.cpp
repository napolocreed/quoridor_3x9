...
// Temporary mask-to-dense-index lookup used only while precomputing symmetry and
// one-wall transitions. Flat open addressing avoids millions of allocator-owned
// nodes and releases its two contiguous arrays cleanly before search starts.
class ConfigIndex {
    std::vector<uint64_t> keys_; // mask + 1; zero is the empty-slot sentinel
    std::vector<uint32_t> vals_;
    size_t mask_ = 0;
    size_t size_ = 0;
public:
    void build(const std::vector<uint64_t>& masks) {
        size_t cap=1;
        while(cap < masks.size()*2) cap <<= 1;
        keys_.assign(cap,0);
        vals_.resize(cap);
        mask_=cap-1;
        size_=0;
        for(uint32_t i=0;i<masks.size();++i) {
            uint64_t stored=masks[i]+1;
            size_t slot=size_t(mix64(masks[i]))&mask_;
    ...
    uint32_t find(uint64_t key) const {
        if(keys_.empty()) return UINT32_MAX;
        uint64_t stored=key+1;
        size_t slot=size_t(mix64(key))&mask_;
        while(true) {
            uint64_t got=keys_[slot];
            if(got==0) return UINT32_MAX;
            if(got==stored) return vals_[slot];
            slot=(slot+1)&mask_;
        }
    }
    size_t size() const { return size_; }
    size_t bytes() const { return keys_.size()*sizeof(uint64_t)+vals_.size()*sizeof(uint32_t); }
    void clear() {
...
class TransTable {
    // Structure-of-arrays storage keeps full 64-bit keys while packing all bounded
    // proof metadata into one u32. This uses 12 bytes per slot instead of 16 and
    // preserves collision soundness by comparing the complete canonical key.
...
public:
    explicit TransTable(unsigned pow2 = 24) {
        ...
        size_t n=size_t(1)<<pow2;
        keys_.assign(n,0);
        meta_.assign(n,pack_meta(WIN_NONE,0,MOVE_NONE));
        mask_=n-1;
    }
    void new_generation() {}
    ...
    bool probe(uint64_t key, int depth, bool &value, uint16_t &hint) const {
        ...
        for(size_t i:{a,b}) {
            ...
            uint32_t m=meta_[i];
            uint16_t bm=best_move(m);
            hint=(bm==MOVE_NONE?0xffff:bm);
            uint8_t wd=win_depth(m), fd=fail_depth(m);
            if(wd!=WIN_NONE && depth>=int(wd)) { value=true; return true; }
            if(depth<=int(fd)) { value=false; return true; }
            return false;
        }
        return false;
    }
    void record(uint64_t key, int depth, bool value, uint16_t move) {
        ...
        size_t i;
        if(keys_[a]==key) i=a;
        else if(keys_[b]==key) i=b;
        else i=quality(a)<=quality(b)?a:b;
        uint8_t wd=WIN_NONE,fd=0;uint16_t bm=MOVE_NONE;
        if(keys_[i]==key) {
            uint32_t old=meta_[i];wd=win_depth(old);fd=fail_depth(old);bm=best_move(old);
        } else keys_[i]=key;
        if(value) wd=uint8_t(std::min<int>(wd,depth));
        else fd=uint8_t(std::max<int>(fd,depth));
        if(wd!=WIN_NONE && fd>=wd) throw std::runtime_error("inconsistent transposition bounds");
        if(move!=0xffff) {
            if(move>0x1fe) throw std::runtime_error("move id exceeds packed TT range");
            bm=move;
        }
...
public:
    ...
    int max_total_walls;
    int order_mode = 1;
    int symmetry_mode = 2; // 0=none, 1=horizontal reflection, 2=plus 180-degree player swap
    bool precompute_landmarks = true; // only required by the optional distance bound
    uint64_t all_cell_mask;

    std::vector<uint64_t> cfg_masks;
    std::vector<uint8_t> cfg_placed;
    ConfigIndex cfg_index;
    std::vector<uint8_t> clear_dirs; // [cfg*N + sq], bits UDLR
    // Store only distances to Player 1's top goal. Player 2 distances are
    // obtained by rotating the board and swapping players, saving one byte per
    // square per wall configuration.
    std::vector<uint8_t> dist_top;    // [cfg*N + sq]
    ...
    Engine(int width, int height, int walls, int ordering = 1, int symmetry = 2, bool landmarks = true)
        ...
        precompute_landmarks = landmarks;
    ...
    void precompute(bool keep_index = true) {
        ...
        // Build the rotation map before distances so Player 2 goal distances can
        // be derived from the single stored Player 1 table.
        auto t2=Clock::now();
        build_symmetry();
        std::cerr << "symmetry time=" << std::chrono::duration<double>(Clock::now()-t2).count() << "s\n";
        auto t3=Clock::now();
        build_board_tables();
        std::cerr << "board tables time=" << std::chrono::duration<double>(Clock::now()-t3).count() << "s\n";
        auto t4=Clock::now();
        build_transitions();
        std::cerr << "transitions=" << trans.size() << " time="
                  << std::chrono::duration<double>(Clock::now()-t4).count() << "s\n";
        if (!keep_index) cfg_index.clear();
        size_t bytes = cfg_masks.size()*8 + cfg_placed.size() + clear_dirs.size() +
            dist_top.size()+landmark_dist.size()+
    ...
    inline uint8_t distance(Player player, uint32_t cfg, uint8_t p) const {
        if(player==P1) return dist_top[size_t(cfg)*N+p];
        return dist_top[size_t(rotate_cfg[cfg])*N+rotate_sq[p]];
    }
    ...
    int generate(const State& s, Player target, Child* out, bool ordered=true) const {
        ...
        if(s.walls[me]>0) {
            ...
            for(uint32_t i=a;i<b;i++) {
                uint32_t nc=trans[i];
                if(distance(P1,nc,s.p[0])==255 || distance(P2,nc,s.p[1])==255) continue;
                ...
                int before_op=distance(Player(op),s.cfg,s.p[op]), after_op=distance(Player(op),nc,s.p[op]);
                int before_me=distance(Player(me),s.cfg,s.p[me]), after_me=distance(Player(me),nc,s.p[me]);
    ...
    int static_eval(const State& s, Player target) const {
        ...
        int dt=distance(Player(t),s.cfg,s.p[t]);
        int do_=distance(Player(o),s.cfg,s.p[o]);
...
private:
    ...
    void build_index() {
        cfg_index.build(cfg_masks);
        std::cerr << "index_storage=" << human_bytes(cfg_index.bytes()) << "\n";
    }
    ...
    void build_board_tables() {
        ...
        clear_dirs.resize(M*N); dist_top.resize(M*N);
        if(precompute_landmarks) landmark_dist.resize(M*N*LANDMARKS);
        else { landmark_dist.clear(); landmark_dist.shrink_to_fit(); }
        ...
        for(size_t ci=0;ci<M;ci++){
            ...
            uint8_t* dd=dist_top.data()+ci*N; std::fill(dd,dd+N,uint8_t(255));
            int head=0,tail=0;
            for(int c=0;c<W;c++){uint8_t p=sq(0,c);dd[p]=0;q[tail++]=p;}
            while(head<tail){uint8_t p=q[head++];uint8_t cd=clear_dirs[ci*N+p];for(int d=0;d<4;d++)if(cd&(1u<<d)){uint8_t z=neigh[p][d];if(z!=255&&dd[z]==255){dd[z]=uint8_t(dd[p]+1);q[tail++]=z;}}}
    ...
    void build_symmetry() {
        ...
        for(uint32_t i=0;i<M;i++){
            uint32_t im=cfg_index.find(transform_mask(cfg_masks[i],true,false));
            uint32_t ir=cfg_index.find(transform_mask(cfg_masks[i],false,true));
            if(im==UINT32_MAX||ir==UINT32_MAX)throw std::runtime_error("symmetry config missing");
            mirror_cfg[i]=im;rotate_cfg[i]=ir;
        }
    }
    void build_transitions() {
        ...
        for(uint32_t i=0;i<M;i++){
            ...
            for(int w=0;w<2*S;w++) if((m&wall_conflict[w])==0){
                uint64_t nm=m|(1ULL<<w); uint32_t it=cfg_index.find(nm); if(it!=UINT32_MAX)trans.push_back(it);
            }
...
class ProofSolver {
    ...
    uint64_t nodes=0, tt_hits=0, cutoffs=0, pawn_table_hits=0, stalemates_seen=0;
...
public:
    struct Result { bool solved=false; Player winner=P1; int depth=-1; uint64_t nodes=0,tt_hits=0,cutoffs=0,pawn_table_hits=0,stalemates_seen=0; double seconds=0; bool timeout=false; };
    ProofSolver(const Engine& eng,unsigned tt_bits,bool bounds=true,bool pawn_table=true,bool transpositions=true):
        e(eng),tt(transpositions?tt_bits:10),use_distance_bound(bounds),use_pawn_table(pawn_table),use_tt(transpositions) {
        std::cerr<<"TT="<<(use_tt?Engine::human_bytes(tt.bytes()):std::string("disabled"))
                 <<" bounds="<<use_distance_bound<<" pawn_table="<<use_pawn_table<<"\n";
    }
    bool probe_tt(uint64_t key,int depth,bool& value,uint16_t& hint) {
        if(!use_tt){hint=0xffff;return false;}
        return tt.probe(key,depth,value,hint);
    }
    void record_tt(uint64_t key,int depth,bool value,uint16_t best_move) {
        if(use_tt) tt.record(key,depth,value,best_move);
    }
    ...
    bool prove(const State& s,Player target,int depth,
        ...
        ++nodes;
        if((nodes&16383ULL)==0 && Clock::now()>=deadline){timed_out=true;throw SearchTimeout{};}
        Player win;
        ...
        if(use_distance_bound) {
            ...
            int graph_d=e.distance(Player(ti),s.cfg,s.p[ti]);
        ...
        if(probe_tt(key,depth,cached,hint)){++tt_hits;return cached;}
        if(use_pawn_table) if(auto po=pawn_only_result(s)) {
            bool exact=(po->second<0)?false:(po->first==target && po->second<=depth);
            record_tt(key,depth,exact,0xffff);
            return exact;
        }
        auto &ch=move_lists[ply];
        int n=e.generate(s,target,ch.data(),true);
        if(n==0){
            // This branch is unreachable on the reported boards if the path
            // preservation invariant holds (see docs/PROOF_SEMANTICS.md).
            // Keep the historical convention explicit, but expose every hit
            // as audit data instead of silently relying on it.
            ++stalemates_seen;
            record_tt(key,depth,false,0xffff);
            return false;
        }
        if(hint!=0xffff) {
        ...
        record_tt(key,depth,result,best);
        return result;
    }
    Result solve_from(State root,double seconds,int max_depth,int start_depth=1,int target_filter=-1) {
        ...
        for(int d=start_depth;d<=max_depth;d++){
            for(Player target:{P1,P2}){
                ...
                uint64_t before=nodes;
                bool ok=false;
                try{ok=prove(root,target,rd,move_lists,0);}catch(const SearchTimeout&){out.timeout=true;goto done;}
        ...
        out.nodes=nodes;out.tt_hits=tt_hits;out.cutoffs=cutoffs;out.pawn_table_hits=pawn_table_hits;out.stalemates_seen=stalemates_seen;out.seconds=std::chrono::duration<double>(Clock::now()-t0).count();
        return out;
    }
    ...
    std::vector<uint16_t> extract_pv(State s,Player target,int depth,double seconds=120.0) {
        // Extract a minimax line from an already-proved bound. On target turns choose a
        // fastest winning child; on opponent turns choose a child with the largest
        // required proof depth. The latter matters: taking the first universal child can
        // produce a trivially short cooperative line that is not representative of the proof.
        deadline=Clock::now()+std::chrono::duration_cast<Clock::duration>(std::chrono::duration<double>(seconds));
        timed_out=false;
        std::vector<uint16_t> line;
        std::vector<std::array<Engine::Child,192>> move_lists(size_t(std::max(1,depth))+2);
        auto required_depth=[&](const State& child,int max_depth)->int {
            Player winner;
            if(e.terminal(child,winner)) return winner==target?0:-1;
            int md=target_relevant_depth(child.turn,target,max_depth);
            int first=(child.turn==target)?1:2;
            for(int d=first;d<=md;d+=2) if(prove(child,target,d,move_lists,1)) return d;
            return -1;
        };
        while(depth>0) {
            Player winner;if(e.terminal(s,winner))break;
            depth=target_relevant_depth(s.turn,target,depth);
            if(depth<=0)break;
            auto &ch=move_lists[0];int n=e.generate(s,target,ch.data(),true);if(n<=0)break;
            int chosen=-1,chosen_need=(s.turn==target?std::numeric_limits<int>::max():-1);
            for(int i=0;i<n;i++) {
                int need=required_depth(ch[i].s,depth-1);
                if(need<0) continue;
                if(s.turn==target) {
                    if(need<chosen_need){chosen=i;chosen_need=need;}
                } else {
                    if(need>chosen_need){chosen=i;chosen_need=need;}
                }
            }
            if(chosen<0)break;
            line.push_back(ch[chosen].move);s=ch[chosen].s;depth=chosen_need;
        }
        return line;
    }
};
...
int main(int argc,char**argv){
    ...
    int W=3,H=9,walls=8,max_depth=80,start_depth=1,tt_bits=24,order_mode=1,target_filter=-1,root_index=-1,root_index2=-1,root_index3=-1,jobs=3,child_depth=-1,scan_root_start=0,scan_root_end=-1;double seconds=60;bool precompute_only=false,list_root=false,list_second=false,list_third=false,parallel_second=false,scan_roots=false,scan_current=false,scan_current_unique=false,use_bounds=true,use_pawn_table=true,use_tt=true,print_pv=false;int symmetry_mode=2;
    for(int i=1;i<argc;i++){
        ...
        else if(a=="--root-index")val(root_index);else if(a=="--root-index2")val(root_index2);else if(a=="--root-index3")val(root_index3);else if(a=="--list-root")list_root=true;else if(a=="--list-second")list_second=true;else if(a=="--list-third")list_third=true;else if(a=="--parallel-second")parallel_second=true;else if(a=="--scan-roots")scan_roots=true;else if(a=="--scan-current")scan_current=true;else if(a=="--scan-current-unique")scan_current=true,scan_current_unique=true;else if(a=="--scan-root-start")val(scan_root_start);else if(a=="--scan-root-end")val(scan_root_end);else if(a=="--jobs")val(jobs);else if(a=="--child-depth")val(child_depth);
        else if(a=="--seconds")val(seconds);else if(a=="--tt-bits")val(tt_bits);else if(a=="--order")val(order_mode);else if(a=="--precompute-only")precompute_only=true;else if(a=="--no-bounds")use_bounds=false;else if(a=="--no-pawn-table")use_pawn_table=false;else if(a=="--no-tt")use_tt=false;else if(a=="--print-pv")print_pv=true;else if(a=="--no-symmetry")symmetry_mode=0;else if(a=="--mirror-only")symmetry_mode=1;
        else if(a=="--help"){std::cout<<"frontier_solver --width 3 --height 9 --walls 8 --seconds 600 --max-depth 120 --tt-bits 25 [--no-bounds] [--no-pawn-table] [--no-tt] [--print-pv] [--no-symmetry|--mirror-only]\n";return 0;}
    }
    try{
        Engine e(W,H,walls,order_mode,symmetry_mode,use_bounds);e.precompute(false);
        ...
        if(scan_roots) {
            if(target_filter<0) throw std::runtime_error("scan-roots requires --target 1 or 2");
            if(child_depth<0) child_depth=max_depth-1;
            std::array<Engine::Child,192> raw{};
            Player target=target_filter==0?P1:P2;
            int nr=e.generate(root,target,raw.data(),true);
            int first=std::max(0,scan_root_start);
            int last=scan_root_end<0?nr-1:std::min(nr-1,scan_root_end);
            if(first>last||first>=nr) throw std::runtime_error("empty root scan range");
            int proved=0,refuted=0,unresolved=0;
            uint64_t aggregate_nodes=0,aggregate_stalemates=0; double aggregate_seconds=0;
            for(int i=first;i<=last;i++) {
                ProofSolver ps(e,tt_bits,use_bounds,use_pawn_table,use_tt);
                auto r=ps.solve_from(raw[i].s,seconds,child_depth,child_depth,int(target));
                aggregate_nodes+=r.nodes;aggregate_stalemates+=r.stalemates_seen;aggregate_seconds+=r.seconds;
                bool win=r.solved&&r.winner==target;
                if(r.timeout)++unresolved; else if(win)++proved; else ++refuted;
                std::cout<<"root_scan index="<<i<<" move="<<label(raw[i].move)
                         <<" proven="<<win<<" timeout="<<r.timeout<<" nodes="<<r.nodes
                         <<" tt_hits="<<r.tt_hits<<" stalemates_seen="<<r.stalemates_seen<<" seconds="<<r.seconds
                         <<" child_depth="<<child_depth<<" total_depth="<<(child_depth+1)<<"\n"<<std::flush;
                // At a target-to-move root, one proved child is a complete bounded proof.
                if(root.turn==target&&win) break;
                // At an opponent-to-move root, one completed false child refutes the bounded proof.
                if(root.turn!=target&&!r.timeout&&!win) break;
            }
            bool bounded_proof=(root.turn==target)?(proved>0):(refuted==0&&unresolved==0);
            std::cout<<"root_scan_result proven="<<bounded_proof<<" child_proved="<<proved
                     <<" child_refuted="<<refuted<<" child_unresolved="<<unresolved
                     <<" aggregate_nodes="<<aggregate_nodes<<" stalemates_seen="<<aggregate_stalemates<<" aggregate_seconds="<<aggregate_seconds
                     <<" total_depth="<<(child_depth+1)<<" range="<<first<<"-"<<last<<"\n";
            return bounded_proof?0:(unresolved?3:1);
        }
        ...
        if(list_third){if(root_index2<0)throw std::runtime_error("list-third requires root-index and root-index2");choose_child(root,-1,true,"third");return 0;}
        if(root_index3>=0){if(root_index2<0)throw std::runtime_error("root-index3 requires root-index and root-index2");root=*choose_child(root,root_index3,false,"third");}
        if(scan_current) {
            ...
            Player target=target_filter==0?P1:P2;
            int nr=e.generate(root,target,raw.data(),true);
            struct Task{int rep;State st;std::vector<int> aliases;};
            std::vector<Task> tasks;
            if(scan_current_unique) {
                std::unordered_map<uint64_t,int,U64Hash> bykey;
                for(int i=0;i<nr;i++) {
                    uint64_t k=e.canonical_key(raw[i].s,target);
                    auto it=bykey.find(k);
                    if(it==bykey.end()){int z=int(tasks.size());bykey[k]=z;tasks.push_back({i,raw[i].s,{i}});}
                    else tasks[it->second].aliases.push_back(i);
                }
            } else {
                for(int i=0;i<nr;i++) tasks.push_back({i,raw[i].s,{i}});
            }
            int first=std::max(0,scan_root_start);
            int last=scan_root_end<0?int(tasks.size())-1:std::min(int(tasks.size())-1,scan_root_end);
            if(first>last||first>=int(tasks.size())) throw std::runtime_error("empty current scan range");
            int proved=0,refuted=0,unresolved=0;
            uint64_t aggregate_nodes=0,aggregate_stalemates=0;double aggregate_seconds=0;
            for(int ti=first;ti<=last;ti++) {
                ProofSolver ps(e,tt_bits,use_bounds,use_pawn_table,use_tt);
                auto r=ps.solve_from(tasks[ti].st,seconds,child_depth,child_depth,int(target));
                aggregate_nodes+=r.nodes;aggregate_stalemates+=r.stalemates_seen;aggregate_seconds+=r.seconds;
                bool win=r.solved&&r.winner==target;
                if(r.timeout)++unresolved;else if(win)++proved;else ++refuted;
                std::cout<<"current_scan task="<<ti<<" rep="<<tasks[ti].rep<<" aliases=";
                for(size_t j=0;j<tasks[ti].aliases.size();j++){if(j)std::cout<<",";std::cout<<tasks[ti].aliases[j];}
                std::cout<<" move="<<label(raw[tasks[ti].rep].move)
                         <<" proven="<<win<<" timeout="<<r.timeout<<" nodes="<<r.nodes
                         <<" tt_hits="<<r.tt_hits<<" stalemates_seen="<<r.stalemates_seen<<" seconds="<<r.seconds
                         <<" child_depth="<<child_depth<<" total_suffix="<<(child_depth+1)<<"\n"<<std::flush;
                if(root.turn==target&&win)break;
                if(root.turn!=target&&!r.timeout&&!win)break;
            }
            bool complete_range=(first==0&&last==int(tasks.size())-1);
            bool bounded_proof=(root.turn==target)?(proved>0):(refuted==0&&unresolved==0&&complete_range);
            bool bounded_refutation=(root.turn==target)?(proved==0&&unresolved==0&&complete_range):(refuted>0);
            std::cout<<"current_scan_result proven="<<bounded_proof<<" refuted="<<bounded_refutation
                     <<" child_proved="<<proved<<" child_refuted="<<refuted<<" child_unresolved="<<unresolved
                     <<" aggregate_nodes="<<aggregate_nodes<<" stalemates_seen="<<aggregate_stalemates<<" aggregate_seconds="<<aggregate_seconds
                     <<" total_suffix="<<(child_depth+1)<<" range="<<first<<"-"<<last
                     <<" tasks="<<tasks.size()<<" raw="<<nr<<" unique="<<scan_current_unique<<"\n";
            return bounded_proof?0:(bounded_refutation?1:(unresolved?3:1));
        }
        if(parallel_second) {
#ifdef _WIN32
            throw std::runtime_error("--parallel-second requires POSIX fork/pipe support");
#else
            ...
            struct Wire{uint64_t nodes,tt_hits,cutoffs,pawn,stalemates;double secs;int solved,winner,depth,timeout;};
            struct Active{pid_t pid;int fd;int task;};std::vector<Active> active;
            int next=0,done=0;bool any_false=false,any_timeout=false;uint64_t aggregate_stalemates=0;
            auto launch=[&](int ti){
                int fds[2];if(pipe(fds))throw std::runtime_error("pipe failed");pid_t pid=fork();if(pid<0)throw std::runtime_error("fork failed");
                if(pid==0){close(fds[0]);ProofSolver ps(e,tt_bits,use_bounds,use_pawn_table,use_tt);auto r=ps.solve_from(tasks[ti].st,seconds,child_depth,std::max(1,child_depth-2),int(target));
                    Wire w{r.nodes,r.tt_hits,r.cutoffs,r.pawn_table_hits,r.stalemates_seen,r.seconds,int(r.solved),int(r.winner),r.depth,int(r.timeout)};(void)!write(fds[1],&w,sizeof(w));close(fds[1]);_exit(0);}
            ...
            while(done<int(tasks.size())){
                ...
                std::cout<<" ok="<<ok<<" proven="<<(ok?w.solved:0)<<" timeout="<<(ok?w.timeout:1)<<" nodes="<<(ok?w.nodes:0)<<" stalemates_seen="<<(ok?w.stalemates:0)<<" seconds="<<(ok?w.secs:0)<<"\n"<<std::flush;
                if(ok)aggregate_stalemates+=w.stalemates;
                if(!ok||w.timeout)any_timeout=true;else if(!w.solved)any_false=true;
            }
            std::cout<<"parallel_result proven="<<(!any_false&&!any_timeout)<<" disproven="<<any_false<<" unresolved="<<any_timeout<<" stalemates_seen="<<aggregate_stalemates<<" total_depth="<<(child_depth+2)<<"\n";
            return (!any_false&&!any_timeout)?0:(any_false?1:3);
#endif
        }
        ProofSolver ps(e,tt_bits,use_bounds,use_pawn_table,use_tt);auto r=ps.solve_from(root,seconds,max_depth,start_depth,target_filter);
        std::cout<<"width="<<W<<" height="<<H<<" walls="<<walls<<" solved="<<r.solved;
        if(r.solved)std::cout<<" winner="<<(r.winner==P1?1:2)<<" depth="<<r.depth;
        std::cout<<" nodes="<<r.nodes<<" tt_hits="<<r.tt_hits<<" cutoffs="<<r.cutoffs<<" pawn_table_hits="<<r.pawn_table_hits<<" stalemates_seen="<<r.stalemates_seen<<" seconds="<<r.seconds<<" timeout="<<r.timeout<<"\n";
        if(print_pv && r.solved) {
            auto pv=ps.extract_pv(root,r.winner,r.depth);
            std::cout<<"pv=";for(size_t i=0;i<pv.size();i++){if(i)std::cout<<" ";std::cout<<label(pv[i]);}std::cout<<"\n";
        }
