...
## Réplication indépendante (1er août, suite) — TERMINÉE côté borne sup
    python3 grind.py upper …   → 18/18 classes prouvées par qsolve (mon solveur séparé)
    16 murs + P(0,0) : profondeur 31 ; P(1,1) : profondeur 33  → J1 gagne en ≤ 35 ✓
    ~1,56 G nœuds comptabilisés (TT partagée entre classes). Détail : replication/*.json

## Certificat de stratégie (pipeline validé)
    Bug trouvé et corrigé : put() de la mémo doit garder la certification au rem MINIMAL
    (sinon cycles — reproducteur : verify_cert.py forensic … ; cf. HANDOFF.md).
    Part H(0,0) ré-extraite puis vérifiée : 2500/2500 lignes adverses aléatoires,
    J1 gagne toujours en ≤ 33 plis. Reste : 15 parts à extraire + vérificateur C++ complet.
