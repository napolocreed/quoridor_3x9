...
### 2.4 Réplication indépendante (mise à jour du 1er août, fin de journée)

Depuis la rédaction initiale : la **borne supérieure est intégralement
répliquée** par mon solveur séparé (`qsolve.cpp`, architecture sans
énumération de configs) — 18/18 classes de réponses prouvées (16 murs et
P(0,0) à profondeur 31, P(1,1) à 33), soit J1 gagne en ≤ 35 plis. La borne
inférieure (« exactement 35 ») reste à rejouer (commande prête). Un
pipeline de **certificat vérifiable** est construit et validé sur une
classe (2 500 lignes adverses aléatoires sans défaite) ; détails et étapes
restantes dans `HANDOFF.md`.

### 2.5 Limites de ma vérification
